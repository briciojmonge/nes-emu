...
impl CPU {
    ...
    fn get_operand_address(&self, mode: AddressingMode) -> u16 {
        match mode{
            ...
            AddressingMode::Indirect_X => {
                ...
                let ptr: u8 =(base as u8).wrapping_add(1) as u16;
            ...
            AddressingMode::NoneAddressing => {
                panic!("mode {:?} does not have an address", mode);
            }
        }
    }
    //
    ...
    fn add_to_reggister_a(&mut self, data:u8){
    ...
    fn sbc(&mut self, mode:&AddressingMode){
        let addr=self.get_operand_address(*mode);
        let data = self.mem_read(addr);
        self.add_to_reggister_a((data as i8).wrapping_neg().wrapping_sub(1) as u8);
    }

    fn adc(&mut self, mode:&AddressingMode){
        let addr=self.get_operand_address(*mode);
        let data = self.mem_read(addr);
        self.add_to_reggister_a(data);
    }
    ...
    //
    pub fn run (&mut self){ // sin finalizar
        ...
        loop{
            ...
            let opcode = opcodes.get(&code).expect(&format!("OpCode {:x} not found", code));
            ...
            match code{
                ...
                /*BNE*/ 0xd0 => self.branch(!self.status.contains(CpuFlags::ZERO)),
                /*BVS*/ 0x70 => self.branch(self.status.contains(CpuFlags::OVERFLOW)),
                /*BVC*/ 0x50 => self.branch(!self.status.contains(CpuFlags::OVERFLOW)),
                /*BPL*/ 0x10 => self.branch(!self.status.contains(CpuFlags::NEGATIVE)),
                /*BMI*/ 0x30 => self.branch(self.status.contains(CpuFlags::NEGATIVE)),
                /*BEQ*/ 0xf0 => self.branch(self.status.contains(CpuFlags::ZERO)),
                /*BCS*/ 0xb0 => self.branch(self.status.contains(CpuFlags::CARRY)),
                /*BCC*/ 0x90 => self.branch(!self.status.contains(CpuFlags::CARRY)),
...
mod test{
    ...
    #[test]
    fn test_0xa9_lda_inmediate_load_data() {
        let mut cpu = CPU::new();
